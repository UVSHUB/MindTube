...
attachBtn.addEventListener('mouseover', () => attachBtn.style.background = '#e8eaed');
attachBtn.addEventListener('mouseout', () => attachBtn.style.background = '#f1f3f4');
    ...
    try {
        ...
        if (response.ok && data.success) {
            ...
            setTimeout(() => {
                ...
                document.getElementById('subjectAreaElement').innerText = "📚 " + (data.subject_area || "General Education");
                document.getElementById('difficultyBadge').innerText = data.difficulty_level || "Intermediate";
                ...
                prereqElement.innerHTML = (data.prerequisites?.length > 0) 
                ...
                objectivesElement.innerHTML = (data.learning_objectives?.length > 0) 
                ...
                conceptsElement.innerHTML = (data.key_concepts?.length > 0) 
                ...
                examplesElement.innerHTML = (data.examples_covered?.length > 0) 
                    ? data.examples_covered.map((ex, idx) => `
                ...
                takeawaysElement.innerHTML = (data.key_takeaways?.length > 0) 
                    ? data.key_takeaways.map(t => `<li style="margin-bottom: 0.5rem;">${t}</li>`).join('') 
...
async function uploadToDrive() {
    try {
        ...
        if (currentAnalysisData.key_concepts?.length > 0) {
            addColorBox('KEY CONCEPTS', [33, 150, 243], [255, 255, 255]);
            currentAnalysisData.key_concepts.forEach(c => addText(`• ${c}`, 10, 'normal'));
            yPosition += 5;
        }
